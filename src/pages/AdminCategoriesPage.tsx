...
const AdminCategoriesPage: React.FC = () => {
  const [categories, setCategories] = useState([
    { id: 1, name: 'Gaming PCs', description: 'High-performance gaming computers', productCount: 24, status: 'Active' },
    { id: 2, name: 'Workstations', description: 'Professional workstations for creators', productCount: 18, status: 'Active' },
    { id: 3, name: 'Servers', description: 'Enterprise and home servers', productCount: 12, status: 'Active' },
    { id: 4, name: 'Accessories', description: 'PC accessories and peripherals', productCount: 42, status: 'Active' },
    { id: 5, name: 'Components', description: 'Individual PC components', productCount: 56, status: 'Active' },
    { id: 6, name: 'Peripherals', description: 'Keyboards, mice, monitors', productCount: 38, status: 'Active' },
    { id: 7, name: 'Networking', description: 'Network equipment and accessories', productCount: 15, status: 'Inactive' },
    { id: 8, name: 'Software', description: 'Software and licenses', productCount: 8, status: 'Active' },
  ]);
  ...
  const handleSaveEdit = () => {
    if (editingId) {
      setCategories(categories.map(cat => 
        cat.id === editingId 
          ? { ...cat, name: editData.name, description: editData.description }
          : cat
      ));
      setEditingId(null);
        ...
        <aside className="w-64 bg-gray-900 border-r border-gray-800 min-h-[calc(100vh-73px)]">
          <nav className="p-4 space-y-1">
            ...
            <div className="sidebar-group">
              ...
              <a href="/admin/products/categories" className="sidebar-item active">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main className="flex-1 p-6">
          ...
          <div className="card p-6">
              ...
              <table className="w-full">
                ...
                <tbody>
                  {categories.map((category) => (
                    <tr key={category.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      <td className="py-3 px-4">
                        <div className="font-medium text-white">#{category.id}</div>
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4">
                        <div className="text-gray-400">{category.productCount} products</div>
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          {editingId === category.id ? (
                            <>
                              <button onClick={handleSaveEdit} className="btn btn-ghost p-1 text-emerald-400">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                </svg>
                              </button>
                              <button onClick={() => setEditingId(null)} className="btn btn-ghost p-1 text-gray-400">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                                </svg>
                              </button>
                            ...
                            <>
                              <button onClick={() => handleEdit(category.id)} className="btn btn-ghost p-1">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                              </button>
                              <button onClick={() => toggleStatus(category.id)} className="btn btn-ghost p-1">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                              </button>
                              <button onClick={() => deleteCategory(category.id)} className="btn btn-ghost p-1 text-red-400">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center justify-between mt-6 pt-6 border-t border-gray-800">
              <div className="text-sm text-gray-400">Showing 1-8 of 8 categories</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            <div className="card p-6">
              ...
              <div className="text-sm text-emerald-400 mt-2">+2 from last month</div>
            </div>
            <div className="card p-6">
              <div className="text-sm text-gray-400 mb-2">Active Categories</div>
              <div className="text-2xl font-bold text-white">{categories.filter(c => c.status === 'Active').length}</div>
              <div className="text-sm text-emerald-400 mt-2">93% active rate</div>
            </div>
            <div className="card p-6">
              <div className="text-sm text-gray-400 mb-2">Total Products</div>
              <div className="text-2xl font-bold text-white">{categories.reduce((sum, cat) => sum + cat.productCount, 0)}</div>
              <div className="text-sm text-emerald-400 mt-2">+24 from last month</div>
            </div>
          </div>
        </main>
      </div>
