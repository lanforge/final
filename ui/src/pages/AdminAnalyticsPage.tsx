  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-[#111111] p-4 rounded-xl border border-white/5 relative overflow-hidden group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#111111] p-5 rounded-xl border border-white/5">
          <h2 className="text-lg font-bold text-white mb-3">Most Popular Pages</h2>
          <div className="space-y-2">
            {stats.popularPages && stats.popularPages.length > 0 ? (
              stats.popularPages.map((page: any, index: number) => (
                <div key={index} className="flex justify-between items-center border-b border-white/5 pb-2 text-sm">
                  <span className="text-gray-300 truncate max-w-[70%]">{page._id}</span>
                  <span className="text-white font-bold">{page.views} views</span>
                </div>
              ))
            ) : (
          ...
          <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
            {sessions && sessions.length > 0 ? (
              sessions.slice(0, 30).map((session: any, index: number) => (
                <div 
                  key={index} 
                  className="flex items-center justify-between bg-black/30 hover:bg-black/50 p-3 rounded-lg border border-white/5 hover:border-emerald-500/30 transition-colors cursor-pointer group"
                  onClick={() => window.location.href = `/admin/analytics/session/${session._id}`}
                >
                  <div className="flex-1 min-w-0 pr-4">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-bold text-white truncate">Session: {session._id.substring(0, 8)}...</span>
                      <span className="text-xs bg-gray-800 text-gray-300 px-1.5 py-0.5 rounded flex-shrink-0">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <FontAwesomeIcon icon={faEye} className="text-gray-600 group-hover:text-emerald-400 transition-colors" />
                  </div>
                </div>
              ))
            ) : (
