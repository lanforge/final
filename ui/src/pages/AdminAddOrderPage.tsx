      ...
      <form onSubmit={handleSubmit} className="admin-card p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2 border-t border-gray-800 pt-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
              <label className="block text-sm font-medium text-gray-400">Product Selection</label>
              <div className="flex flex-wrap items-center gap-4">
                <label className="flex items-center space-x-2 text-sm text-gray-400 cursor-pointer">
                  <input
                    ...
                    className="text-emerald-500 focus:ring-emerald-500/20 bg-gray-900 border-gray-700"
                  />
                  <span>Standard</span>
                </label>
                <label className="flex items-center space-x-2 text-sm text-gray-400 cursor-pointer">
            ...
            {productType === 'custom-product' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">Custom Product Name *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">Custom Product Price ($) *</label>
                  <input
                    type="number"
            ...
            {productType === 'custom-build' && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-white font-bold mb-3 border-b border-gray-800 pb-2">Option 1: Verify Existing Build ID</h3>
                  <div className="flex items-end gap-4">
                    <div className="flex-1">
                      <label className="block text-sm font-medium text-gray-400 mb-2">Custom Build ID</label>
                      <input
                        type="text"
                        name="customBuildId"
                        className="admin-input w-full bg-gray-900 border-gray-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl"
                  ...
                  {customBuildData && (
                    <div className="mt-4 bg-gray-800 p-4 rounded-xl border border-emerald-500/30">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="text-white font-medium">{customBuildData.name || 'Custom Build'}</h4>
                          <p className="text-gray-400 text-sm">Status: {customBuildData.status}</p>
                          <p className="text-gray-400 text-sm">{customBuildData.parts?.length || 0} items</p>
                        </div>
                        <div className="text-right">
                          <div className="text-emerald-400 font-bold">${customBuildData.total?.toFixed(2)}</div>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                {!customBuildData && (
                  <div className="pt-4">
                    <h3 className="text-white font-bold mb-3 border-b border-gray-800 pb-2">Option 2: Inline Custom Build</h3>
                    <p className="text-sm text-gray-400 mb-4">Select components directly here. A 20% system integration fee will be applied.</p>
                    ...
                    {(Object.values(selectedParts).some(v => v && v !== 'none') || selectedCaseFans.some(f => f && f !== 'none')) && (
                      <div className="mt-4 p-4 bg-gray-800/50 rounded-xl border border-gray-700">
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-gray-400">Inline Build Component Total (Cost):</span>
                        ...
                        <div className="flex justify-between items-center text-sm mt-1">
                          <span className="text-gray-400">System Integration & Validation (20%):</span>
          ...
          <div className="md:col-span-2 border-t border-gray-800 pt-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-medium text-gray-400">Shipping Options & Insurance</h3>
              <button 
                type="button" 
                onClick={fetchRates} 
            ...
            {shippoRates.length > 0 ? (
              <div className="space-y-3 mb-6">
                {shippoRates.map((rate) => (
                  <label key={rate.objectId} className={`flex items-center space-x-3 p-3 bg-gray-900 rounded-xl border cursor-pointer transition-colors ${selectedShippingRate === rate.objectId ? 'border-emerald-500' : 'border-gray-700 hover:border-emerald-500/50'}`}>
                    <input 
                      type="radio" 
                      name="shippingRate" 
                      value={rate.objectId} 
                      ...
                      className="text-emerald-500 focus:ring-emerald-500/20 bg-gray-800" 
                    />
                    <div className="flex-1">
                      <div className="text-white font-medium">{rate.title || rate.provider}</div>
                      <div className="text-sm text-gray-400">{rate.estimatedDays ? `${rate.estimatedDays} days` : 'Standard delivery'}</div>
                    </div>
                    <div className="text-emerald-400 font-bold">${parseFloat(rate.amount).toFixed(2)}</div>
                  </label>
                ))}
              </div>
            ) : (
            ...
            <div className="mt-4">
              <label className="flex items-center space-x-3 p-3 bg-gray-900 rounded-xl border border-gray-700 cursor-pointer hover:border-emerald-500/50 transition-colors">
                <input 
                  type="checkbox" 
                  checked={addInsurance} 
                  onChange={(e) => setAddInsurance(e.target.checked)} 
                  className="text-emerald-500 focus:ring-emerald-500/20 bg-gray-800 rounded" 
                />
                <div className="flex-1">
                  <div className="text-white font-medium">Add Shipping Insurance</div>
                  <div className="text-sm text-gray-400">Protect order during shipping (1.25% of subtotal + shipping)</div>
                </div>
        ...
        <div className="flex justify-between items-center pt-6 mt-6 border-t border-gray-800">
          <div className="text-white font-medium">
            <span className="text-gray-400 mr-2">Total Order Value:</span>
