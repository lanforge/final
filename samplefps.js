...
const LANFORGE_PERFORMANCE_ENGINE_VERSION = "1.0.0";
 ...
 */
function getPerformanceIndex(cpuName, gpuName) {
  const normalizedCPU = normalizeCPU(cpuName);
  const normalizedGPU = normalizeGPU(gpuName);

  const cpuScore = CPU_SCORES[normalizedCPU] || 120;
  const gpuScore = GPU_SCORES[normalizedGPU] || 150;
 ...
 */
function getResolutionAdjustedIndex(cpuName, gpuName, resolution = "1080p") {
  const normalizedCPU = normalizeCPU(cpuName);
  const normalizedGPU = normalizeGPU(gpuName);
 ...
 */
function clampFPS(gameKey, resolution, fps) {
  const caps = {
 ...
 */
function estimateGameFPS(cpuName, gpuName, gameKey, resolution = "1080p") {
  ...
  let min = Math.round(index * multiplier.min);
  let max = Math.round(index * multiplier.max);
 ...
 */
function estimateAllFPS(cpuName, gpuName) {
  const normalizedCPU = normalizeCPU(cpuName);
  const normalizedGPU = normalizeGPU(gpuName);
  ...
  const result = {
    ...
    cpu: {
      ...
      score: CPU_SCORES[normalizedCPU] || 120
    },
    gpu: {
      input: gpuName,
      normalized: normalizedGPU,
      score: GPU_SCORES[normalizedGPU] || 150
    },
  ...
  for (const resolution of RESOLUTIONS) {
    result.resolutions[resolution] = {
      adjustedPerformanceIndex: getResolutionAdjustedIndex(cpuName, gpuName, resolution),
      games: {}
    };
 ...
 */
function getShortPerformanceSummary(cpuName, gpuName) {
  ...
  return {
    ...
    highlights: {
      "1080p": {
        fortniteCompetitive:
          estimates.resolutions["1080p"].games.fortniteCompetitive.display,
        warzoneHigh:
          estimates.resolutions["1080p"].games.warzoneHigh.display,
        valorantCompetitive:
          estimates.resolutions["1080p"].games.valorantCompetitive.display
      },
      "1440p": {
        fortniteCompetitive:
          estimates.resolutions["1440p"].games.fortniteCompetitive.display,
        warzoneHigh:
          estimates.resolutions["1440p"].games.warzoneHigh.display,
        valorantCompetitive:
          estimates.resolutions["1440p"].games.valorantCompetitive.display
      },
      "4k": {
        fortniteCompetitive:
          estimates.resolutions["4k"].games.fortniteCompetitive.display,
        warzoneHigh:
          estimates.resolutions["4k"].games.warzoneHigh.display,
        valorantCompetitive:
          estimates.resolutions["4k"].games.valorantCompetitive.display
      }
    }
  };
}
...
if (typeof window !== "undefined") {
  window.LANForgePerformanceEngine = {
    version: LANFORGE_PERFORMANCE_ENGINE_VERSION,
    CPU_SCORES,
    GPU_SCORES,
    GAME_MULTIPLIERS,
    normalizeCPU,
    normalizeGPU,
    getPerformanceIndex,
    getResolutionAdjustedIndex,
    estimateGameFPS,
    estimateAllFPS,
    getShortPerformanceSummary,
    disclaimer: LANFORGE_FPS_DISCLAIMER
  };
}
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    LANFORGE_PERFORMANCE_ENGINE_VERSION,
    CPU_SCORES,
    GPU_SCORES,
    GAME_MULTIPLIERS,
    normalizeCPU,
